---
name: specs
description: Interactive spec and app-surface co-evolution. Discuss a change with the user via AskUserQuestion, stress-test edge cases, then update specs (docs/specs/) and production-owned app surfaces in lockstep.
---

# Specs Skill

You are running an **interactive design session**. The product has
specs (`docs/specs/`) and production-owned app surfaces under `app/`.
The specs are the source of truth, and this skill is the canonical way
to evolve specs and visible app contracts together.

Your job: **talk with the user, stress-test the idea against real
edge cases, and only then edit the specs and app surfaces** so they
...
2. **Specs are the source of truth; app surfaces follow them.** If a
   change lands in app UI/API behavior but not the spec, you've created
   drift. Every app-surface edit has a matching spec edit in the same
   turn (or a reasoned note saying why not — e.g., the UI picks one
   concrete affordance where the spec intentionally leaves room).
...
- Introducing a new entity, role, or flow and wiring it into `app/web/`
  plus the relevant FastAPI routes or production-owned fixtures.
...
2. LOAD CONTEXT
   ...
   - Read the matching app pieces (React pages/components, FastAPI
     routers, fixtures, seed scripts)
...
5. DECIDE
   ...
   │
   ▼
6. APPLY (specs + app surfaces in lockstep)
   - Edit the spec sections (surgical, preserve structure)
   - Update the React app, FastAPI route, fixture, or seed data so the
     product reflects the new rule
   - Run the relevant app/web build, type-check, or backend tests when
     the change is non-trivial
   │
   ▼
7. REVIEW + HANDOFF
   - Summarise: what changed in specs, what changed in app surfaces,
     what is deliberately left open, and what the user should eyeball
...
| **Migration** | Existing fixtures, seed scripts, or demo data need updating so they stay consistent? |
...
- **Fixtures / seeds**: update production-owned fixtures, seed scripts,
  or demo data so they reflect the new rule. Do not leave stale
  fixtures that contradict the spec.
- **API**: update the FastAPI routers / response models under `app/`
  so the server returns what the spec now promises.
...
### Keeping specs and app surfaces in lockstep

If you edit a spec rule, the corresponding app surface must change in the
same session. If you edit an app surface, the spec must change too (or
you must justify why the app surface is intentionally freer than the
spec — e.g., the spec says "at least one method"; the UI picks one).
...
- [ ] Type-check / build passes on the React app if touched.
...
- Edit the relevant FastAPI router — add a `POST
  /work-orders/{id}/reassign` handler scoped to owner-manager.
- Edit the relevant seed/demo fixture — seed one reassigned order so
...
- **Editing app surfaces without touching specs**, or vice versa. Drift
  starts here.
...
- **Inventing new roles or scopes** without reflecting them in §03
  and §05.
- **Hardcoding strings** in new React app code — use the i18n seam.
