...
class RoleGrantRepository(Protocol):
    ...
    def has_live_grants_in_workspace(self, *, workspace_id: str, user_id: str) -> bool:
        ...
        with the ``delegating_user_inactive`` problem type (cd-ljvs).
        The check is workspace-scoped — a live grant in a *sibling*
    ...
    def has_live_grants_anywhere(self, *, user_id: str) -> bool:
        ...
        non-revoked grant across every workspace returns 401 with the
        ``subject_user_inactive`` problem type (cd-ljvs). The check is
