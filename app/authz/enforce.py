...
:func:`app.authz.dep.Permission` FastAPI dependency factory;
service-layer callers (background workers, CLI, agent) call
:func:`require` directly.

This module is **pure-domain**: it does not import any FastAPI or
:mod:`app.api` symbols. The router-side wrapper (the ``Permission``
dependency factory) lives in :mod:`app.authz.dep` so that domain
modules importing ``from app.authz import require`` do not transitively
pull in :mod:`app.api.deps` and trip the import-linter
"Domain forbids handlers" contract.
