...
interface NormalizedTaskDetail {
  task: NormalizedTask;
  property: Property | null;
  instructions: Instruction[];
  inventory_effects: ResolvedInventoryEffect[];
...
export default function TaskDetailPage() {
  ...
  const completeButtonText = (() => {
    if (complete.isPending) return "Completing...";
    if (uploadingPhoto) return "Uploading photo...";
    if (task.photo_evidence === "required" && photoEvidenceIds.length === 0) {
      return (
...
function normalizeTaskDetail(payload: ApiTask | TaskDetailResponse): NormalizedTaskDetail {
  ...
  return {
    ...
    inventory_effects:
      response.inventory_effects ?? effectsFromConsumption(rawTask.inventory_consumption_json ?? {}),
  };
}
...
function normalizeTask(task: ApiTask): NormalizedTask {
  ...
  return {
    ...
    scheduled_start:
      task.scheduled_for_utc ?? task.scheduled_start ?? task.scheduled_for_local ?? task.created_at ?? "",
    estimated_minutes: task.duration_minutes ?? task.estimated_minutes ?? 30,
    priority: task.priority,
    status: stateToStatus(state),
    checklist: (task.checklist ?? []).map(normalizeChecklistItem).filter((item) => item.label),
...
function effectsFromConsumption(consumption: Record<string, number>): ResolvedInventoryEffect[] {
  return Object.entries(consumption).map(([itemRef, qty]) => ({
    item_ref: itemRef,
    kind: "consume",
    qty,
    item_id: null,
...
function commentToMessage(comment: CommentPayload): AgentMessage {
  return {
