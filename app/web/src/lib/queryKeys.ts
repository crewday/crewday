...
// DECISIONS.
//
// Central key factory so invalidations stay type-safe. Mirrors
// `mocks/web/src/lib/queryKeys.ts` verbatim so every component port
// compiles against the same surface; the cd-qdsl real impl keeps this
// exact shape and only swaps the consuming API client over.
