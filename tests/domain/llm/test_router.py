    ...
    def test_thinking_level_uses_model_default(
        ...
        try:
            provider_model = seed_provider_model(
                db_session,
                provider_model_id="01HWA00000000000000000THD1",
                model_capabilities=["chat", "function_calling"],
            )
            ...
            assert pick.thinking_level == "medium"
        finally:
            reset_current(token)

    def test_thinking_level_provider_model_override_wins(
        self, db_session: Session, clock: FrozenClock
    ) -> None:
        ws = seed_workspace(db_session)
        ctx = build_context(ws.id)
        ...
        try:
            provider_model = seed_provider_model(
                db_session,
                provider_model_id="01HWA00000000000000000THD2",
                model_capabilities=["chat", "function_calling"],
            )
            model = db_session.get(LlmModel, provider_model.model_id)
            assert model is not None
            ...
            assert pick.thinking_level == "high"
        finally:
            reset_current(token)

    def test_thinking_level_assignment_override_wins(
        self, db_session: Session, clock: FrozenClock
    ) -> None:
        ws = seed_workspace(db_session)
        ctx = build_context(ws.id)
        ...
        try:
            provider_model = seed_provider_model(
                db_session,
                provider_model_id="01HWA00000000000000000THD3",
                model_capabilities=["chat", "function_calling"],
            )
            model = db_session.get(LlmModel, provider_model.model_id)
            assert model is not None
            ...
            assert pick.thinking_level == "disabled"
        finally:
            reset_current(token)
