...
- **LLM** (§11): `llm_provider`, `llm_model`, `llm_provider_model`,
  ...
  `disabled | low | medium | high` default; `llm_assignment` may
  carry a nullable `thinking_level_override` with the same enum and
  NULL meaning "inherit the model default".
...
| archived_at        | tstz?       | Soft-archive timestamp set by owner danger-zone archive/delete (§16) or `crewday admin workspace archive` (§13). Archived workspaces are inactive for normal selection and multi-workspace decisions, keep owning their slug while the row exists, and remain recoverable by a future restore/unarchive flow before any scheduled purge runs. |
| delete_requested_at | tstz?      | Owner Delete request timestamp. Set separately from `archived_at` so ordinary archive and scheduled deletion remain distinguishable. Repeated Delete requests keep the original value. |
| purge_after        | tstz?       | Hard-delete deadline, normally `delete_requested_at + 14 days` for the §16 archived deletion grace period. The purge worker deletes only rows whose deadline has elapsed; slug reservation starts after that purge, not when this field is set. |
| created_at         | tstz        |                                                                                                |
...
available. Archived workspaces do not rely on this table for
normal slug ownership; their live `workspaces.slug` row keeps the
slug unavailable until the workspace is recovered or purged.

**Reasons:**

- `archived` — legacy or operator-created reservation for a slug
  released from an archived workspace. The owner danger-zone archive
  path does not create this row; the archived workspace keeps owning
  its slug.
