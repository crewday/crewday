...
| `users.reset_passkey`                   | `workspace`                    | `owners`                      | ✅ | §03 — gates `POST /users/{id}/reset_passkey` (§"Owner-initiated worker passkey reset"). Owners-only by default because the action mails a fresh enrolment link to the worker AND a non-consumable notification copy to the owner; managers can still re-issue an ordinary recovery link via `POST /users/{id}/magic_link`. |
...
| `places.share`                          | `property`                     | `owners, managers`            | —  | §04/§22 — create, update, or revoke `property_workspace` share rows from the property sharing surface. Domain service still enforces owner-workspace authority. |
| `stays.read`                            | `workspace`, `property`        | `owners, managers`            | —  | §04/§12 — gates manager stay surfaces: iCal feed metadata, reservation listings, stay bundles, and active welcome-link management state. Guest welcome pages are anonymous and token-gated instead of action-gated. |
| `stays.manage`                          | `workspace`, `property`        | `owners, managers`            | —  | §04/§12 — gates mutating stay operations: iCal feed registration/update/disable/delete/poll, stay-bundle regeneration, and guest welcome-link issue/revoke. |
