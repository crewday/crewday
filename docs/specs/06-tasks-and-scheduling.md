# 06 — Tasks and scheduling

Tasks are the heart of the system. Every interaction — for employees,
managers, and agents — either produces tasks, consumes tasks, or
reports on tasks.

## Task kinds (logical, not a column)

1. **One-off.** A single task with a due date. Not attached to a
   schedule.
2. **Scheduled instance.** A concrete task materialized from a
   schedule. Has `schedule_id` and `originally_scheduled_for`.
3. **Turnover.** Generated as part of a turnover bundle after a stay's
   check-out (§04).
4. **Triggered.** Generated by a non-calendar trigger: inventory low,
   issue report conversion, agent action, webhook.
5. **Subtask.** A child of another task (used sparingly — prefer
   checklist items).

All five are rows in the same `tasks` table; the kind is derivable
from `schedule_id IS NULL`, `turnover_bundle_id IS NULL`, etc.

## Task template

A reusable definition. Scheduling a recurring task means linking a
template to a schedule. Ad-hoc tasks may or may not reference a
template; one-off cleanings often do not.

| field                          | type      | notes                                 |
|--------------------------------|-----------|---------------------------------------|
| id                             | ULID PK   |                                       |
| household_id                   | ULID FK   |                                       |
| name                           | text      | "Weekly kitchen deep clean"           |
| description_md                 | text      |                                       |
| role_id                        | ULID FK?  | default role expected to do it        |
| duration_minutes               | int       | hint only                             |
| property_scope                 | enum      | `any | one | listed`                  |
| listed_property_ids            | ULID[]    |                                       |
| area_scope                     | enum      | `any | one | listed | derived`        |
| listed_area_ids                | ULID[]    |                                       |
| checklist_template_json        | jsonb     | list of checklist items w/ optional guest_visible |
| photo_evidence                 | enum      | `disabled | optional | required`      |
| linked_instruction_ids         | ULID[]    | §07                                   |
| priority                       | enum      | `low | normal | high | urgent`        |
| inventory_consumption_json     | jsonb     | SKU → qty expected, see §08           |
| llm_hints_md                   | text      | free text to help agents understand intent |
| deleted_at                     | tstz?     |                                       |

## Schedule

A schedule says "create a task from this template on these dates at
these times". Stored as RFC 5545 RRULE plus a property-local timezone.

| field              | type    | notes                                       |
|--------------------|---------|---------------------------------------------|
| id                 | ULID PK |                                             |
| household_id       | ULID FK |                                             |
| name               | text    | "Villa Sud pool — Saturdays 09:00"          |
| template_id        | ULID FK |                                             |
| property_id        | ULID FK |                                             |
| area_id            | ULID FK?|                                             |
| default_assignee   | ULID FK?| employee — see assignment logic             |
| rrule              | text    | RFC 5545, e.g. `FREQ=WEEKLY;BYDAY=MO,TH`    |
| dtstart_local      | timestamp | local time (property tz)                  |
| duration_minutes   | int     |                                             |
| rdate_local        | text    | RDATE override list                         |
| exdate_local       | text    | EXDATE skip list                            |
| active_from        | date    |                                             |
| active_until       | date?   |                                             |
| paused_at          | tstz?   |                                             |
| deleted_at         | tstz?   |                                             |

### UI expose-levels

Manager UI shows two panels:

- **Simple:** frequency (weekly/daily/monthly), days of week, time,
  duration. Generates an RRULE under the hood.
- **Advanced:** raw RRULE field (with live "next 5 occurrences"
  preview) + RDATE/EXDATE editors.

Behavior is identical — both write the same RRULE column. We **do not**
paraphrase RRULE into our own DSL.

### Generation

A worker job `generate_task_occurrences` runs every hour:

1. For each active schedule whose last materialized occurrence is
   within **14 days** of the horizon, compute the next RRULE
   occurrences up to `now + 30 days`.
2. Apply `exdate_local` and then `rdate_local`.
3. For each new occurrence not yet materialized, create a `task` row
   with `originally_scheduled_for = occurrence`, copying fields from
   the template and the schedule defaults.
4. Run the assignment algorithm (below).
5. Apply blackout dates (employee leave, property closures) → may
   leave tasks **unassigned**.

The horizon is configurable per household. Shrinking it doesn't
un-create already-created tasks; growing it does a backfill.

### Pause / resume

A schedule may be paused (`paused_at`). Paused schedules do not
generate new occurrences; existing ones remain. Resume clears the
field and the next tick picks up.

### Deleting and editing

- Editing a schedule only affects **future unmaterialized**
  occurrences. Already-materialized tasks are untouched unless the
  manager uses **"Apply to existing tasks"** in the dialog, which
  patches tasks with `status IN ('scheduled', 'pending')` only.
- Deleting a schedule soft-deletes the row and cancels all
  `status = scheduled` tasks linked to it (sets `status = cancelled`,
  `cancellation_reason = 'schedule deleted'`).

## Task row

| field                        | type      | notes                                         |
|------------------------------|-----------|-----------------------------------------------|
| id                           | ULID PK   |                                               |
| household_id                 | ULID FK   |                                               |
| template_id                  | ULID FK?  |                                               |
| schedule_id                  | ULID FK?  |                                               |
| turnover_bundle_id           | ULID FK?  |                                               |
| parent_task_id               | ULID FK?  |                                               |
| property_id                  | ULID FK   |                                               |
| area_id                      | ULID FK?  |                                               |
| title                        | text      | copied from template; editable                |
| description_md               | text      |                                               |
| priority                     | enum      |                                               |
| state                        | enum      | `scheduled | pending | in_progress | completed | skipped | cancelled | overdue` |
| scheduled_for_local          | tstz      | local at property tz, plus UTC mirror         |
| scheduled_for_utc            | tstz      |                                               |
| due_by_utc                   | tstz?     | SLA; default = `scheduled_for + duration`     |
| duration_minutes             | int       |                                               |
| photo_evidence               | enum      | copied from template; editable                |
| checklist_json               | jsonb     | snapshot from template at creation            |
| linked_instruction_ids       | ULID[]    |                                               |
| expected_role_id             | ULID FK?  |                                               |
| assigned_employee_id         | ULID FK?  | null = unassigned                             |
| completed_at                 | tstz?     |                                               |
| completed_by_employee_id     | ULID FK?  |                                               |
| completion_note_md           | text?     |                                               |
| skipped_reason               | text?     |                                               |
| cancellation_reason          | text?     |                                               |
| llm_generated                | bool      | created by an agent                           |
| llm_correlation_id           | ULID?     | if llm_generated                              |
| created_at / updated_at      | tstz      |                                               |
| deleted_at                   | tstz?     |                                               |

### State machine

```
  scheduled ──► pending ──► in_progress ──► completed
     │                                          ▲
     │                                          │
     └───────────► skipped  ◄──────────── cancelled
                  (reason)        (reason)
          │
          └──► overdue (auto, after due_by_utc)
```

- `scheduled` → `pending` happens at `scheduled_for_utc - 1h` (or
  immediately for one-offs created with a past `scheduled_for`).
- `pending` → `in_progress` is optional; employees may go directly
  to `completed`.
- `completed` is terminal.
- `overdue` is a soft state: it is set by a worker if `due_by_utc` is
  past and state is still `scheduled | pending | in_progress`. It
  reverts to the underlying state on manual state change.

## Assignment algorithm

Given a new task with `expected_role_id` and `property_id`:

1. If the task has `schedule.default_assignee`, use that.
2. Else: candidate = employees who:
    - have `employee_role` with `role_id = expected_role_id`,
    - whose assignment covers `property_id` (or role is
      unrestricted), and
    - are not on leave on `scheduled_for_local`.
3. If exactly one candidate, assign them.
4. If more than one candidate:
    - Prefer the employee with the **fewest tasks** in the 7-day window
      around `scheduled_for_local` at the same property.
    - Break ties by rotation: pick the employee whose last task at
      this property is the oldest.
5. If zero candidates, leave unassigned; surface in the daily digest.

Managers and agents can always override. Auto-assignment is a tie-
breaker, not a policy.

## Blackout dates and employee leave

Each employee has a `leave` ledger: `{id, employee_id, starts_on,
ends_on, reason, approved_at, approved_by}`. During generation, if a
candidate is on leave on the occurrence date they are removed from the
candidate set. Property-level blackouts (renovation week, owner
vacation) are modeled as property `closures` that prevent any tasks
being generated at that property for the range.

## Completing a task

From the employee PWA: tap → "Mark done". If `photo_evidence =
required`, the camera picker opens; the file is uploaded and linked
before state flips. If `checklist_required` capability is on, all
items must be ticked.

Server-side:

1. Validate state transition.
2. Record `completed_at`, `completed_by_employee_id`.
3. Apply `inventory_consumption_json` as `inventory_movement` rows
   unless the capability is off (§08).
4. Insert `task_completion` row (a tombstone used for history even if
   the task is later edited).
5. Fire `task.completed` webhook.
6. Audit log.

## Evidence

Photos, optional notes, optional checkbox completion state. Stored
per completion as `task_evidence {kind: photo|note|checklist_state,
file_id?, text?}`. Files go through the `storage` abstraction (local
FS by default, §15).

## Comments

Threaded. Manager and assigned employee (and any other employee on the
same task's property with messaging.comments) can post. Comments are
Markdown; `@mentions` resolve to household members. Notifications are
via email (§10).

## Skipping and cancellation

- `skip`: employee action, requires a reason (capability gated).
  Counts as "not done" in reporting but does not raise an issue. Used
  for "not needed this week" (guest left early, etc.).
- `cancel`: manager action only. Reason required.

Both are terminal states.

## Searching

- Full-text on title, description, completion_note, comments (§02
  portability).
- Facets: state, property, area, role, assignee, priority, date range.

## Turnover template

A property has a `turnover_template_id` pointer: a special kind of
task template that generates a **bundle** of tasks on stay checkout.
The bundle's tasks share `turnover_bundle_id`, allowing grouping in
the UI. Example: "Strip and wash linens", "Reset kitchen", "Check
inventory", "Publish guest welcome link for next stay".

Bundle generation respects the cleaning window between check-out and
the next check-in; SLA `due_by` defaults to `next_checkin_at - 30m` or
`checkout + 4h`, whichever is earlier.

## Natural-language intake (agent)

`POST /api/v1/tasks/from_nl` (see §11) takes a string, resolves
property/area/role/schedule using the LLM, and returns a **preview**
(no row created). The caller confirms via `POST /api/v1/tasks/from_nl/
commit` with the preview id. Idempotent.

## CLI (examples)

```
miployees tasks list --property prop_01… --on 2026-04-21
miployees tasks create "airport pickup" --property prop_01… \
                       --role driver --when 'tomorrow 06:30 Europe/Paris'
miployees tasks assign <task-id> --to emp_…
miployees tasks complete <task-id> --photo ./done.jpg --note 'all good'
miployees schedules add --template tmpl_… --property prop_… \
                        --rrule 'FREQ=WEEKLY;BYDAY=MO,TH' --at 09:00
miployees schedules preview --template tmpl_… --rrule '...' --for 30d
```
