...
| `passkey_admin_revoked` | `admin_revoke_passkey` (app/auth/passkey.py) | per-user | Owner-initiated single-credential revoke via `DELETE /admin/users/{user_id}/passkeys/{credential_id}` (§03 "Additional passkeys", §12). The target user's sessions are invalidated with this distinct cause so forensics can separate owner action from self-service revoke. |
...
| `logout`             | `post_logout` route handler (app/api/v1/auth/logout.py), via `invalidate_for_user` | per-user | User-initiated sign-out via `POST /api/v1/auth/logout` (§03 "Sessions", §12). Invalidates every active session for the caller — including the caller's own — so a stolen sibling cookie (other tabs, saved `curl` copy) stops authenticating on the next request. The handler is best-effort: a missing / invalid / expired cookie still returns 204 + clearing Set-Cookie but writes no audit row (nothing happened server-side). |
...
user identified by `occurrence.created_by_user_id`, and (b) members of the
...
`is_personal = false OR created_by_user_id = caller_id OR caller_is_owner`.
