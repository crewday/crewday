...
- **System doc (agent doc).** Code-shipped Markdown that the chat
  ...
  card phrasing, hand-off rules). Stored in the `agent_doc` table
  (§02), one of the hash-self-seeded tables (§02 "Hash-self-seeded
  tables"); seeded from `app/agent_docs/*.md`. Operators override
  per deployment via `/admin/agent-docs` or
  `crewday deploy agent-docs edit <slug>`.
...
- **TCO (total cost of ownership).** The all-in cost of an asset:
  ...
  property. See §21.
- **Template.** Disambiguation — "template" is used in several
  senses in this repo; they are **distinct mechanisms** and
  retention/admin rules differ:
  - **Task template** (§06). Reusable task definition (title,
    checklist, duration, role, evidence policy). Structured domain
    data, not code-defaulted text.
  - **Stay-lifecycle rule template** (§04). The task template
    pointed at by a stay-lifecycle rule; drives
    `before_checkin` / `after_checkout` / `during_stay` bundles.
  - **Hash-self-seeded template** (§02 "Hash-self-seeded tables").
    Code-shipped text default with DB override — prompts
    (`llm_prompt_template`, §11) and agent docs (`agent_doc`,
  ...
  - **Action-confirmation `summary` template** (§12
    `x-agent-confirm`). OpenAPI-annotation-authored; pre-rendered
    once and frozen on the `agent_action` row.
...
- **Prompt library.** DB-backed system-prompt store
  ...
  tables** (§02) — code default seeds the row, unmodified prompts
  auto-upgrade when code changes, admin-customised prompts are
  preserved. Edited from the `/admin/llm` "Prompts" slide-over.
  See §11 "Prompt library".
- **Hash-self-seeded table.** Shared primitive (§02 "Hash-self-seeded
  tables") for code-shipped text defaults that operators may
  override per deployment without losing upgrades: code declares
  the default, the DB carries the customisation, and
  `sha256(default)[:16]` in `default_hash` reconciles the two on
  boot. Unmodified rows auto-upgrade when the code default changes;
  customised rows are preserved. v1 callers: `llm_prompt_template`
  (§11), `agent_doc` (§02 + §11 "Agent knowledge tools"). Not used
  by email templates (§10, filesystem), WhatsApp templates (§23,
  Meta-registered), task templates (§06, structured data), or
  action-confirmation `summary` templates (§12, OpenAPI
  annotation). Revision retention: `retention.template_revisions_days`.
