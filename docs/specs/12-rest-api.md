...
POST   /api/v1/auth/passkey/signup/register/start  # self-serve signup passkey ceremony (§03 "Self-serve signup"); mints the registration challenge bound to the signup_session_id returned by /signup/verify (no user row exists yet); no session cookie yet. Parallel surface to /signup/passkey/start — see cd-ju0q for the dual-surface consolidation.
POST   /api/v1/auth/passkey/signup/register/finish # verifies the attestation and persists the first passkey row for the user_id the signup service reserved; no session is minted here. Parallel to /signup/passkey/finish (cd-ju0q).
