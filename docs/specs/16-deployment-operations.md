...
| `CREWDAY_PUBLIC_SITE_URL`  | -                              | Optional separate marketing-site URL. When set, an anonymous browser visiting the app bare root `/` is redirected there; when unset, the app keeps the local login fallback so self-hosters do not need `site/`. |
