...
| `CREWDAY_ICAL_ALLOW_PRIVATE_ADDRESSES` | `0`                 | **Dev / e2e ONLY — DO NOT enable in production.** Disables the §04 "SSRF guard" private-address rejection inside `IcalValidatorConfig` so a feed URL whose host resolves to loopback / RFC 1918 / link-local is accepted at registration. Sole supported caller is the e2e stack (`docker-compose.dev.yml` with the documented Playwright e2e environment), which needs the GA journey 3 test (cd-zxvk) to register a feed against an in-cluster ICS server. Every other validator gate (scheme, DNS-rebind pin, redirects, body cap, timeout) still applies. cd-xr652. |
