...
| `CREWDAY_PUBLIC_SITE_URL`  | -                              | Optional marketing-site URL or same-origin public route. When set, an anonymous browser visiting the app bare root `/` is redirected there; when unset, the app keeps the local login fallback so self-hosters do not need `site/`. Managed production normally uses an absolute public-site origin; the dev stack defaults to `/mocks/landing/` so `dev.crew.day` stays on the dev host. |
