...
function installFetch(opts: {
  ...
  const spy = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
    ...
    if (pathname === "/api/v1/task_templates" && (initRecord.method ?? "GET") === "GET") {
      const next = listQueue.shift();
      if (!next) {
        // Default to the most recent state once the queue is empty.
        return jsonResponse({ data: [initial], next_cursor: null, has_more: false });
      }
      return jsonResponse(next.body, next.status ?? 200);
    }
    if (pathname === "/api/v1/work_roles") {
      return jsonResponse({ data: ROLES, next_cursor: null, has_more: false });
    }
    if (
      pathname === `/api/v1/task_templates/${initial.id}` &&
      initRecord.method === "PATCH"
    ) {
      const next = patchQueue.shift();
      if (!next) {
        // Default 200 echo of the request body, useful when callers
        // only want to assert the call count, not the response shape.
...
describe("TemplatesPage checklist reorder", () => {
  it("reorders the React Query cache optimistically on drop", async () => {
    ...
    try {
      ...
      // Drain the debounce so the PATCH fires before teardown.
      await act(async () => {
  ...
  it("debounces PATCH to a single request across a multi-drop burst", async () => {
    ...
    try {
      ...
      // Cross the 400 ms debounce boundary; exactly one PATCH should fire.
      await act(async () => {
        await vi.advanceTimersByTimeAsync(500);
      });
      await waitFor(() => {
      ...
      // [first, second, third] -> drop first onto third -> [second, third, first]
      // -> drop second onto first (now last) -> [third, first, second].
  ...
  it("rolls the order back on a 4xx response", async () => {
    ...
    try {
      ...
      // After the 422 the cache restores the snapshot taken before the
      // mutation; the rendered list mirrors that state.
      await waitFor(() => {
        const cached = client.getQueryData<{ data: TaskTemplate[] }>(
          qk.taskTemplates(),
        );
